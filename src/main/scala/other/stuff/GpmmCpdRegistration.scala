...
class GpmmCpdRegistration[D: NDSpace, DDomain[A] <: DiscreteDomain[A]](gpmm: PointDistributionModel[D, DDomain], lambda: Double = 2, w: Double = 0, max_iterations: Int = 100)(
  implicit
...
class GpmmBcpdRegistration[D: NDSpace, DDomain[A] <: DiscreteDomain[A]](
  ...
  def registrationMethod(
    ...
    initialGPMM: DenseVector[Double],
    initialTrans: SimilarityTransformParameters[D]): (DenseVector[Double], SimilarityTransformParameters[D]) = {
  ...
  def register(
    ...
    transformationType: GlobalTranformationType = SimilarityTransforms,
    initialGPMM: DenseVector[Double] = defaultInitialGPMMPars,
    initialTrans: SimilarityTransformParameters[D] = defaultTransformationPars): (DenseVector[Double], SimilarityTransformParameters[D]) = {
...
class GpmmSpecialICPRegistration(
  ...
  def registrationMethod(
    ...
    transformationType: GlobalTranformationType,
    initialGPMM: DenseVector[Double],
    initialTrans: SimilarityTransformParameters[_3D]): (DenseVector[Double], SimilarityTransformParameters[_3D]) = {
    bcpd.Registration(tolerance, transformationType: GlobalTranformationType, initialGPMMpars = initialGPMM, initialTransformation = initialTrans)
  }
  ...
  def register(
    ...
    transformationType: GlobalTranformationType = SimilarityTransforms,
    initialGPMM: DenseVector[Double] = defaultInitialGPMMPars,
    initialTrans: SimilarityTransformParameters[_3D] = defaultTransformationPars): (DenseVector[Double], SimilarityTransformParameters[_3D]) = {
    registrationMethod(tolerance, transformationType, initialGPMM, initialTrans)
  }
}

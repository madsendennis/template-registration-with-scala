...
import breeze.linalg.{DenseMatrix, diag, sum, trace}
...
import scalismo.geometry.{EuclideanVector, NDSpace, _1D, _2D, _3D}
...
//      val lowRankGP = LowRankGaussianProcess.approximateGPCholesky(reference, gp, relativeTolerance = relativeTolerance, interpolator = TriangleMeshInterpolator3D[EuclideanVector[_3D]]())
  ...
  implicit object gpmm1Dpoints extends GPMM[_1D, UnstructuredPointsDomain] {
    override def construct(reference: UnstructuredPointsDomain[_1D], kernel: MatrixValuedPDKernel[_1D], relativeTolerance: Double): PointDistributionModel[_1D, UnstructuredPointsDomain] = {
  ...
  implicit object gpmm2Dpoints extends GPMM[_2D, UnstructuredPointsDomain] {
    override def construct(reference: UnstructuredPointsDomain[_2D], kernel: MatrixValuedPDKernel[_2D], relativeTolerance: Double): PointDistributionModel[_2D, UnstructuredPointsDomain] = {
      val gp = GaussianProcess[_2D, EuclideanVector[_2D]](kernel)
      val lowRankGP = LowRankGaussianProcess.approximateGPCholesky(reference, gp, relativeTolerance = relativeTolerance, interpolator = NearestNeighborInterpolator())
  ...
  implicit object gpmm3Dpoints extends GPMM[_3D, UnstructuredPointsDomain] {
    override def construct(reference: UnstructuredPointsDomain[_3D], kernel: MatrixValuedPDKernel[_3D], relativeTolerance: Double): PointDistributionModel[_3D, UnstructuredPointsDomain] = {
      val gp = GaussianProcess[_3D, EuclideanVector[_3D]](kernel)
      val lowRankGP = LowRankGaussianProcess.approximateGPCholesky(reference, gp, relativeTolerance = relativeTolerance, interpolator = NearestNeighborInterpolator())

...
object step6_simpleCompletionICP extends App {
  ...
  val completedPath = new File(data.completed, "icpRealign_pca")
  completedPath.mkdirs()

  println(s"Experiment output folder: ${completedPath}")
  ...
  //  val targetFile = targetFiles.head //.find(f => f.getName.equals("VSD.case_2.101148.stl")).get

...
object LoadTestData {
  ...
  def modelAndTarget(targetIndex: Int = 0, truncate: Int = 200): (StatisticalMeshModel, String, TriangleMesh[_3D], TriangleMesh[_3D], File) = {

//    val modelFile = new File(generalPath, "faceGPmodel_200c.h5")
    val modelFile = new File(generalPath, "model2017-1_face12_nomouth.h5")
    val model = StatisticalModelIO.readStatisticalMeshModel(modelFile, "shape").get.truncate(truncate)
    ...
    val logPath = new File(generalPath, s"log${truncate}_withIPE")
    if (!logPath.exists()) {
      logPath.mkdir()
    }

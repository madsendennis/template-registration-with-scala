...
object BfmFittingThesisIPE {

  def main(args: Array[String]) {
    scalismo.initialize()
    val completedPath = new File(generalPath, "completedIPE")
...
//        val faceIndex = 7 // 0.2: 2, 4, 7, 0.1: 2, 6
    ...
    val faceIndexes = Seq(4,5,6,7,8,9)
//    val faceIndexes = Seq(0,1,2,3)
...
//      val ui = ScalismoUI(s"BFM-icp-fitting ${faceIndex}")
      ...
      val proposalCP1 = MixedProposalDistributions.mixedProposalICP(decGPMM, targetMeshPartial, Seq(), targetLms, projectionDirection = ModelSampling, tangentialNoise = 100.0, noiseAlongNormal = 3.0, stepLength = 0.1)
      val proposalCP2 = MixedProposalDistributions.mixedProposalICP(decGPMM, targetMeshPartial, Seq(), targetLms, projectionDirection = ModelSampling, tangentialNoise = 1.0, noiseAlongNormal = 1.0, stepLength = 0.5)
      val proposalCP3 = MixedProposalDistributions.mixedProposalICP(model, decTarget, Seq(), targetLms, projectionDirection = TargetSampling, tangentialNoise = 100.0, noiseAlongNormal = 3.0, stepLength = 0.1)
      val proposalCP4 = MixedProposalDistributions.mixedProposalICP(model, decTarget, Seq(), targetLms, projectionDirection = TargetSampling, tangentialNoise = 1.0, noiseAlongNormal = 1.0, stepLength = 0.5)
      ...
      val proposal = MixtureProposal.fromProposalsWithTransition(Seq((0.20, proposalCP1), (0.15, proposalCP2), (0.20, proposalCP3), (0.15, proposalCP4), (0.2, proposalPose), (0.1, proposalRandom)): _ *)
//      val proposal = MixtureProposal.fromProposalsWithTransition(Seq((0.35, proposalCP1), (0.35, proposalCP3), (0.2, proposalPose), (0.1, proposalRandom)): _ *)
...
//      )
      ...
      val evaluator = ProductEvaluators.proximityAndIndependentBoundary(model, targetMeshPartial, evaluationMode = ModelToTargetEvaluation, uncertainty = 0.1, numberOfEvaluationPoints = numberOfEvaluationPoints, boundaryAware = true)
      ...
      def fitting(model: StatisticalMeshModel, targetMesh: TriangleMesh3D, evaluator: Map[String, DistributionEvaluator[ModelFittingParameters]], proposal: ProposalGeneratorWithTransition[ModelFittingParameters], numOfIterations: Int, showModel: Option[StatisticalMeshModelViewControls], log: File, initialParameters: Option[ModelFittingParameters] = None): ModelFittingParameters = {
        ...
        val samplingRegistration = new SamplingRegistration(model, targetMesh, showModel, modelUiUpdateInterval = 10, acceptInfoPrintInterval = 50)
...
//      ui.show(finalGroup, model.instance(cpdFit), "cpd").opacity = 0.0

      val logObj = new JSONAcceptRejectLogger[ModelFittingParameters](new File(targetLogFile.toString.replace(".json", " copy.json")))
      val bestLogPars = logObj.getBestFittingParsFromJSON


      val initFitPars = bestLogPars //ModelFittingParameters.zeroInitialization(modelInit)//.copy(shapeParameters = ShapeParameters(cpdFit))

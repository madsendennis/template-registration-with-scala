...
object AffineICPRegistration extends App {
  ...
  import api.registration.utils.PoseRegistrator.AffineRegistrator3D

  val affineTrans: SimilarityTransformation[_3D] = TranslationAfterScalingAfterRotation3D(Translation3D(EuclideanVector3D(50.0, 20.0, 30.0)), Scaling3D(1.2), Rotation3D(0.1, 0.0, 0.0, Point3D(0, 0, 0)))
  ...
  val reg = new RigidICPRegistration[_3D, TriangleMesh](template, 100)

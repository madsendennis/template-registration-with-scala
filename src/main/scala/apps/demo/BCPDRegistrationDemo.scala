...
object BCPDRegistrationDemo extends App {
  ...
  val rigidTrans: RigidTransformation[_3D] = TranslationAfterRotation3D(Translation3D(EuclideanVector3D(20.0, 5.0, 5.0)), Rotation3D(0.4, 0.0, 0.0, Point3D(0, 0, 0)))

  val template = MeshIO.readMesh(new File("data/femur_reference.stl")).get.operations.decimate(100)
  val target = MeshIO.readMesh(new File("data/femur_target.stl")).get.operations.decimate(100)//.transform(rigidTrans)
  ...
  val myKernel = GaussianKernel[_3D](100)
  val bcpd = new BCPDRegistration(template, target, w = 0.0, lambda=1.0, gamma = 1.0, k=1000.0, kernel = myKernel, max_iterations = 100)

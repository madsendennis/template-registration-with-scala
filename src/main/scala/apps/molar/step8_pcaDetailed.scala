...
object step8_pcaDetailed {
  implicit val random: Random = Random(1024)

  def main(args: Array[String]) {
    scalismo.initialize()

    val alignedMeshesPath = data.registeredMeshesCrownICP.listFiles(_.getName.endsWith(".ply"))

    println(s"Number of registered meshes: ${alignedMeshesPath.length}")
    ...
    StatisticalModelIO.writeStatisticalMeshModel(pca, data.pcaDetailed)
  }
}

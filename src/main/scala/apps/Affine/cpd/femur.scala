...
import api.registration.AffineCPDRegistration
...
import scalismo.transformations.{
  ...
  RotationAfterScalingAfterTranslation3D,
  Scaling,
  Scaling3D,
...
object femur extends App {
  ...
  val rigidTrans: SimilarityTransformation[_3D] =
    TranslationAfterScalingAfterRotation3D(Translation3D(EuclideanVector3D(50.0, 20.0, 30.0)), Scaling3D(2), Rotation3D(0.1, 0.0, 0.0, Point3D(0, 0, 0)))
  ...
  val affineCPD = new AffineCPDRegistration[_3D, TriangleMesh](source, lambda = 10, beta = 100, w = 0.0) // target, source ... Source is moving!!!
  val finalMesh = affineCPD.register(target)
  val ui = ScalismoUI()
